...
我们测量世界，测量结果总会因为两个主要原因存在变动。一个主要原因是世界很大，人类的谁知能力有限。对于一个物理系统，我们总有不了解的因素在影响它们。因此，一个物理系统的输出结果要受到两类因素的影响，一类是我们要控制的因素，一类是我们不控制的因素。我们要控制的因素对系统的输出结果的影响是主要的。但还有一些不能控制的因素，这又分为两部分。一部分是我们针对物理系统建模过程中，有意识忽略而不控制的因素，比如在低速条件下我们忽略了相对认的影响，只用牛顿力学去解决物理系统的运动学问题。一种是我们前面反复强调的我们并没有掌握的某些影响因素的知识。这两类未加控制的因素必定会给影响系统的输出。因此，我们反复测量系统时，每一次的测量值并不一定相同，它总会表现为由机理模型决定的输出加上变动的影响。即：

$$
Y=X+\epsilon
$$

这就是我们测量系统的概率模型。而概率是我们用来量化这个变动的工具。

在研究概率时，我们必须理解等似然事件。在承认测量值会发生变动的情况下，我们所执行的每一次测量就是一次随机事件。随机实验的所有取值构成随机实验的样本空间。样本空间的集合组成了随机事件。等似然性指的是重复随机实验是相互独立的，它所输出的第一个结果的概率是相等的。在这个基础上，依据简单的集合操作和排列组合所提供的计数技术，我们可以分析任何事件发生的概率。所以随机事件的概率是某一事件发生的频率，这是概率的一种解释。当然概率还有第二种解释，即是人的主观自信度。比如：今天下雨的可能性有30%，这就是一种自信度。

我们可以用随机变量来描述概率，随机变量本身是一种函数，它将随机事件映射成一个数值。随机变量可以是离散的，也可以是连续的。离散的随机变量可以用概率质量函数来描述，连续随机变量可以用概率密度函数描述。同时它们都可以用累积概率分布函数来描述。
随机变量的概率质量(密度)函数决定了它的分布。

在概率世界，有两个基本的随机实验特别重要。一个是由中心极限定理控制的随机实验，只要独立同分布的随机实验重复次数足够大，那么它的分布就是正态分布。还有一种随机实验就是伯努利判决随机实验。由伯努利随机判决实验，我们可以推导出二项式分布的概率密度函数。当二项式分布的伯努利判决试验次数趋向于无穷大时，形成了泊松分布，这时的随机实验就是泊松过程。所以，当伯努利判决随机实验的次数足够大，可以用正态分布来近似它们的概率。它们之间的关键就是独立同分布的实验次数足够大。在这个前提条件下，可以直接就认为二项式分布和泊松分布是正态分布。只是在近似时要加上一定的修正因子。同样是泊松过程，我们还可以由此推导出另外一种指数分布随机变量的概率密度函数，它与离散随机变量的几何分布之间有十分密切的联系。

本教材中介绍的概率分布是从众多物理系统中抽象出来的，具有很好的普遍性。它们底层的随机过程是理解和选择使用它们的关键。由于这些随机分布特别重要，所以我们在介绍它们时，往往忽略了它们底层的基础。但是，我们在理解由底层随机过程推导随机变量的过程中，可以学会从特殊到一般的抽象思维方法，这一点是我们不应该忽略的，我们在本教材中也利用文字叙述和实例反复强调了的。这种概率思维的方法是统计思想的基础，需要我们认真思索和理解，并在实践中反复应用。
