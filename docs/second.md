...
!!!note "样本空间"
    随机实验的所有可能的输出集合称为实验的**样本空间**。样本空间用符号$\mathcal{S}$表示。

抛硬币这样的随机实验，其样本空间非常简单，即$\mathcal{S}= \{＋1，－1\}$(+1表示正面，-1表示反面)。混凝土强度的样本空间为正实数区间，即$\mathcal{S}\in [0, +\infty]$。这两个实例的样本空间取值有所不同，前者是取离散值，即在实数空间中的聚值并不连续；后一个实例则完全不同，混凝土强度可以是正实数空间的任意一个值，样本空间取连续实数值。

!!!note "离散和连续样本空间"
    如果样本空间只包含有限的或可数无限的输出值，则该样本空间是**离散样本空间**。如果样本空间包含一个实数区间(无论该区间是有限区间还是无限区间)，则样本空间是**连续样本空间**。

在这个定义中，我们用到了集合的可数无限大的概念，不了解的读者可以研究学习集合论的有关知识。